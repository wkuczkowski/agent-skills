...
Reasoning (section 2.1). An instruction to show, echo or reproduce its reasoning triggers a `reasoning_extraction` refusal. Ask for the result and its evidence, never for the thinking.
...
Output (section 3.3 and 3.5). It writes fewer progress updates during long tool-calling turns and uses less chat formatting than earlier models. Remove anti-formatting language from older documents; if updates are wanted, ask for a one-line opener and a short standalone recap. At low effort it searches less and answers from memory more; raise the effort or add a verification nudge for lookups.
...
- The no-op test in `theory.md` has a higher bar on both models: instructions that were needed against laziness or under-triggering on older models now over-trigger. Run the document without the line before keeping it.
...
- Neither needs to be told to be thorough, to verify, or to think step by step. A stronger leading word replaces a weak one; a default behaviour is left unsaid.
